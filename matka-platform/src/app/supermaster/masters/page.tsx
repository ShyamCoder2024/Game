...
import { Users, Plus, Eye, Lock, Ban, CheckCircle2 } from 'lucide-react';
...
export default function SMMastersPage() {
    ...
    const columns: Column<MasterRow>[] = [
        ...
        { key: 'balance', label: 'Balance', isCurrency: true },
        {
            key: 'deal_percentage',
            label: 'Deal %',
            render: (row: MasterRow) => <span>{row.deal_percentage}%</span>,
        },
            ...
            render: (row: MasterRow) => (
                <div className="flex items-center gap-1">
                    ...
                    <button
                        ...
                        title="Withdraw"
                    >W</button>
                    <button className="w-7 h-7 rounded bg-blue-500 text-white text-xs font-bold hover:bg-blue-600 transition-colors" title="View">
                        <Eye size={14} className="mx-auto" />
                    </button>
                    <button className="w-7 h-7 rounded bg-amber-500 text-white text-xs font-bold hover:bg-amber-600 transition-colors" title="Change Password">
                        <Lock size={14} className="mx-auto" />
                    </button>
                    <button
                        onClick={() => setBlockTarget({ id: row.user_id, name: row.name, status: row.status })}
                        className={`w-7 h-7 rounded text-white text-xs font-bold transition-colors ${row.status === 'active' ? 'bg-gray-500 hover:bg-gray-600' : 'bg-emerald-500 hover:bg-emerald-600'}`}
