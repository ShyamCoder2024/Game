...
export default function MasterSettlementPage() {
    ...
    const columns = [
        ...
        { key: 'name', label: 'Name' },
        { key: 'lena_hai', label: 'लेना है', render: (row: SettlementRow) => <span className={row.lena_hai > 0 ? 'text-green-600 font-semibold' : 'text-gray-400'}>{row.lena_hai > 0 ? `₹${row.lena_hai.toLocaleString('en-IN')}` : '—'}</span> },
        { key: 'dena_hai', label: 'देना है', render: (row: SettlementRow) => <span className={row.dena_hai > 0 ? 'text-red-600 font-semibold' : 'text-gray-400'}>{row.dena_hai > 0 ? `₹${row.dena_hai.toLocaleString('en-IN')}` : '—'}</span> },
        { key: 'le_liya', label: 'ले लिया', render: (row: SettlementRow) => <span className={row.le_liya > 0 ? 'text-blue-600 font-semibold' : 'text-gray-400'}>{row.le_liya > 0 ? `₹${row.le_liya.toLocaleString('en-IN')}` : '—'}</span> },
        { key: 'net', label: 'Net', render: (row: SettlementRow) => <span className={row.net > 0 ? 'text-green-600 font-bold' : row.net < 0 ? 'text-red-600 font-bold' : 'text-gray-500'}>{row.net > 0 ? '↑' : row.net < 0 ? '↓' : ''} ₹{Math.abs(row.net).toLocaleString('en-IN')}</span> },
