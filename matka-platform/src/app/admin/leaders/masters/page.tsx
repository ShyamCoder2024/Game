...
// Masters management page — same pattern as Super Masters
...
export default function MastersPage() {
        ...
        },
        {
            ...
            render: (r) => (
                <div className="flex items-center justify-center gap-1">
                    ...
                    <button className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-500" title="View"><Eye size={16} /></button>
                    <button className="p-1.5 hover:bg-amber-50 rounded-lg text-amber-600" title="Password"><Lock size={16} /></button>
                </div>
            ),
        },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <DataTable title="Masters" columns={columns} data={data} loading={loading}
                totalItems={total} page={page}
                onPageChange={(p) => { setPage(p); fetchData(p); }}
                onSearch={(s) => fetchData(1, s)} grandTotal={grandTotal} rowKey={(r) => r.id}
                actions={<Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white h-9" onClick={() => setCreateOpen(true)}><UserPlus size={14} className="mr-1" />Add Master</Button>}
            />
