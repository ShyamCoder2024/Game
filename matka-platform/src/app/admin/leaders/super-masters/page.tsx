...
// B11: Added edit member modal with full field editing
...
export default function SuperMastersPage() {
    ...
    const fetchData = useCallback(async (p = 1, search = '') => {
        ...
        try {
            const res = await api.get<Member[]>('/api/leaders/list', {
                page: String(p), limit: '20', role: 'supermaster', search,
            });
    ...
    return (
        <div className="space-y-6">
            ...
            <DataTable<Member>
                ...
                actions={
                    <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white h-9" onClick={() => setCreateOpen(true)}>
                        <UserPlus size={14} className="mr-1" />Add SM
                    </Button>
                }
            />
            ...
            <CreateAccountDialog open={createOpen} onClose={() => setCreateOpen(false)} onSuccess={() => fetchData(page)} defaultRole="supermaster" />
            {selectedMember && (
                <CoinTransferDialog
                    open={transferOpen}
